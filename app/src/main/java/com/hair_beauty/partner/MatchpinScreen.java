...
public class MatchpinScreen extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        progressDialog = new ProgressDialog(MatchpinScreen.this);
        sharedPreferenceClass = new SharedPreferenceClass(MatchpinScreen.this);
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(!InputValidation.isEditTextHasvalue(newpin)){
                    Toast.makeText(MatchpinScreen.this, "Please generate a PIN", Toast.LENGTH_SHORT).show();
                }
                else if(!InputValidation.isPinLengthCheck(newpin)){
                    Toast.makeText(MatchpinScreen.this, "Please enter minimum 4 numbers.", Toast.LENGTH_SHORT).show();
                }
                else if(!InputValidation.isEditTextHasvalue(confirmpin)){
                    Toast.makeText(MatchpinScreen.this, "Please confirm yourPIN", Toast.LENGTH_SHORT).show();
                }
                else if (!InputValidation.isPinMatches(newpin,confirmpin)){
                    Toast.makeText(MatchpinScreen.this, "PIN does not match.", Toast.LENGTH_SHORT).show();
                }
                else{
                    createPin();
                }
...
If the  old pin and given pin are same it shows this : {"message":"Create a new pin , the pin you enter is the old one"}*/
                    ...
                    Toast.makeText(MatchpinScreen.this, jsonObject.getString("message"), Toast.LENGTH_SHORT).show();

                    if (jsonObject.getString("message").equals("Pin created successfully")) {

                        sharedPreferenceClass.setValue_string("PINSTATUS","CREATE");

                        startActivity(new Intent(MatchpinScreen.this,EnterpinScreen.class));
                        finish();
            ...
            public void onErrorResponse(VolleyError error) {
                if (error.getMessage() == null) {
                    if (i < 3) {
                        ...
                        progressDialog.dismiss();
                        Toast.makeText(MatchpinScreen.this, "Check your network connection.",
                                Toast.LENGTH_LONG).show();
                    }
                } else
                    Toast.makeText(MatchpinScreen.this, error.getMessage(), Toast.LENGTH_LONG).show();
            }
        }) {
            @Override
        ...
        RequestQueue requestQueue = Volley.newRequestQueue(MatchpinScreen.this);
